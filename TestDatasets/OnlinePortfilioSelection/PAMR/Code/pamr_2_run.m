function [run_ret total_ret day_ret] = pamr_2_run(fid, data, epsilon, C, tc, opts)
% Copyright 2010 by Li Bin.  
% Mean Reversion Passive agressive for Portfolio Selection, run file
% Input
...
%      otps            - environment control variable
%
% Ouput
...
for t = 1:1:T,
    ...
    x_bar = sum(data(t, :)) / N;
    denominator = (data(t, :)-x_bar)*(data(t, :)-x_bar)'+0.5/C;
...
% Debug Information
fprintf(fid, 'PAMR-2(%.2f, %d, %.4f), Final return: %f\n', epsilon, C, tc, run_ret);
fprintf(fid, '-------------------------------------\n');
fprintf(1, 'PAMR-2(%.2f, %d, %.4f), Final return: %f\n', epsilon, C, tc, run_ret);
fprintf(1, '-------------------------------------\n');
