% "PURE" negative log predictive probability objective function for Gaussian 
% process  regression with some utility dependencies from GPML v4 code by Carl  
% Edward Rasmussen and Hannes Nickisch.
%
% Usage of function:
%
% function [log_prob, dlog_prob] = obj_func_lp(hyp_, i_, m_, c_, l_, xt, yt, xs, ys)
%
...
function [log_prob, d_log_prob] = obj_func_lp_pure(hyp_, i_, m_, c_, l_, xt, yt, xs, ys)   
    % get negative log probabilities
    [~, ~, ~, ~, log_prob]  = gp(hyp_, i_, m_, c_, l_, xt, yt, xs, ys);
    log_prob                = - log_prob;
    % pack gp definition to access derivative function (via finite differences)
    gpDef{1}                = i_;
    gpDef{2}                = m_;
    gpDef{3}                = c_;
    gpDef{4}                = l_;
